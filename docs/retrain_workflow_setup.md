...
| **Report refusal and stop** *(only runs if refused)* | Prints `::warning::Promotion refused...` plus the exact reason from `promote_model.py`, then stops cleanly | This IS the "working correctly, but nothing to promote this week" outcome — see Part 3 below. No PR opens. `artifacts/serving/` is untouched. |
...
| **Open PR** *(only runs if promoted)* | A new PR appears against `main`, branch `scheduled-retrain/staging`, titled "Scheduled retrain: new candidate promoted to Staging", body = the metrics-diff table | A permissions error here (403/token) means the repo's Actions settings don't allow this token to open PRs — check **Settings → Actions → General → Workflow permissions**, needs "Read and write permissions" + "Allow GitHub Actions to create and approve pull requests". |
