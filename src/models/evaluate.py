...
def per_race_table(y_true, y_prob, race_ids) -> pd.DataFrame:
    """
    One row per race:
...
def evaluate_all(y_true, y_prob, race_ids) -> dict[str, float]:
    """
...
def evaluate_by_season(y_true, y_prob, race_ids, years) -> pd.DataFrame:
    ...
    for year in sorted(np.unique(years)):
        mask = years == year
        rows[int(year)] = evaluate_all(y_true[mask], y_prob[mask], race_ids[mask])
