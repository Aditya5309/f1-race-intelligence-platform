...
re-invokes materialize_and_score()), and the plain-Python exception types
this module raises for app/api.py to map to HTTP status codes
(RaceAlreadyHasResult, ValueError for a bad/out-of-horizon/naive-as_of
request, RuntimeError for unavailable materialization data) — the actual
HTTP status codes those map to are covered end-to-end in
tests/test_api.py.
